...
$CHECK_COUNT = 3    # Ban after this many failures in search period.     Default: 5
...
function pickupBanDuration { 
	if (Test-Path $BannedIPLog) { 
		get-content $BannedIPLog | % { 
			$IP = $_.split(" ")[0]
			$Setting = $_.split(" ")[1]
			if ((!$BannedIPs.ContainsKey($_.split(" ")[0])) -AND ($Setting -ge $CHECK_COUNT)) { $BannedIPs.Add($IP, $Setting) }
		}			
...
function getBanDuration ($IP) {	
	...
	$BanSetting = [math]::max($Setting - $CHECK_COUNT, 0)
	if ($BanSetting) {
		$BanDuration = [math]::min([math]::pow(5, $BanSetting) * 60, $MAX_BANDURATION)
	}
	else { $BanDuration = 0 }
	debug "IP $IP has the new setting of $setting"
	if (Test-Path $BannedIPLog) { clear-content $BannedIPLog } else { $a = New-Item -Path $BannedIPLog -ItemType file }
 ...
 #bedobedo
	...
	select-string $RegexIP -input $TheEvent.message -AllMatches | foreach { foreach ($a in $_.matches) {
			...
			if ($SelfList -match $IP) { debug "Whitelist of self-listed IPs! Do nothing. ($IP)" }
				...
				debug "$($TheEvent.LogFile) Log Event captured: ID $($RecordID), IP $IP, Event Code $($TheEvent.EventCode)"							
				actioned "$($TheEvent.LogFile) Log Event captured: ID $($RecordID), IP $IP, Event Code $($TheEvent.EventCode)"
				jail_lockup $IP
				unban_old_records
			}
		}
